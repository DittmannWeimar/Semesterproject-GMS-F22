...
void loop() {
  ...
  led_enabled = (bool)(sin(millis() / ((1.0 / led_frequency)) * 1000.0) > 0);
  ledcWrite(0, (float)(led_enabled) * led_power);
...
void handle_setting(const uint8_t * mac, const uint8_t *incomingData, int len) {
  ...
  if (settingMessage.setting == 0) {
  	led_power = settingMessage.newValue;
  }
  if (settingMessage.setting == 1) {
  	led_frequency = settingMessage.newValue;
  }
}
